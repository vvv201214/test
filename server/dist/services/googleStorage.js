"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.getJsonDataFromGCS = exports.deleteFileFromGCS = exports.uploadToGCS = void 0;
const storage_1 = require("@google-cloud/storage");
const credentilsNew = JSON.parse(JSON.stringify({ "type": "service_account",
    "project_id": "bamboo-century-376510",
    "private_key_id": "60f280b497d5dab4e828dfe14443fed9bfeb95d4",
    "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
    "client_email": "jk-backend@bamboo-century-376510.iam.gserviceaccount.com",
    "client_id": "107044988980394359934",
    "auth_uri": "https://accounts.google.com/o/oauth2/auth",
    "token_uri": "https://oauth2.googleapis.com/token",
    "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
    "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/jk-backend%40bamboo-century-376510.iam.gserviceaccount.com" }));
const CONFIGX = {
    credentials: {
        private_key: credentilsNew.private_key,
        client_email: credentilsNew.client_email,
    }
};
const storage = new storage_1.Storage(CONFIGX);
const bucket = storage.bucket('jk-test-docs');
const uploadToGCS = (file) => __awaiter(void 0, void 0, void 0, function* () {
    console.log('inside func gcs');
    const fileName = `${Date.now()}-${file.originalname}`;
    const options = {
        destination: fileName,
        resumable: false,
        metadata: {
            contentType: file.mimetype
        }
    };
    console.log('error?');
    // await bucket.upload(file.buffer, options);
    yield bucket.file(fileName).save(file.buffer, { contentType: file.mimetype });
    console.log(`uploaded`);
    // Get the URL of the uploaded file
    // let fileRef = storage.bucket(bucketName).file(fileName);
    const [url] = yield bucket.file(fileName).getSignedUrl({
        action: 'read',
        expires: '03-09-2491'
    });
    //   console.log(url);
    // Delete the file from GCS
    //   await bucket.file(fileName).delete();
    // Return the URL to the client
    return fileName;
});
exports.uploadToGCS = uploadToGCS;
const deleteFileFromGCS = (fileName) => __awaiter(void 0, void 0, void 0, function* () {
    yield bucket.file(fileName).delete();
});
exports.deleteFileFromGCS = deleteFileFromGCS;
const getJsonDataFromGCS = (fileName, bucketName) => __awaiter(void 0, void 0, void 0, function* () {
    const file = bucket.file(`results/output-1-to-1.json`);
    const contents = yield file.download();
    console.log('contents', contents);
    const data = JSON.parse(contents.toString());
    console.log(data);
    return data;
});
exports.getJsonDataFromGCS = getJsonDataFromGCS;
